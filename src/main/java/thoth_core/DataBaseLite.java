...
import thoth_core.db_lite_structure.DBLiteStructure;
...
public class DataBaseLite {
    ...
    public void readDataBase()
        ...
        List<Table> collect = structure.getTables()
                .stream()
                .filter(table -> !table.getType().equals(StructureDescription.TableTypes.SYSTEM_TABLE.getType()))
                .collect(Collectors.toList());
    ...
    private void setDataTable(Table table, List<HashMap<String, Object>> data) {
        ...
        try {
            StructureDescription.TableTypes tableType = StructureDescription.TableTypes.valueOf(table.getType());
            DBData.getInstance()
