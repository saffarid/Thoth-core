...
import thoth_core.db_data.db_data_element.properties.Orderable;
...
public class Orders
    ...
    public Orders() {
        super();
        setName(StructureDescription.Orders.TABLE_NAME);
    }
    ...
    public Map< String, List< HashMap<String, Object> > > convertToMap(List<? extends Identifiable> list) {
        ...
        for (Identifiable identifiable : list) {
            ...
            map.put(StructureDescription.Orders.CUSTOMER_ID, orderable.getPartner().getId());
            map.put(StructureDescription.Orders.PROJECT_ID, orderable.getProjectable().getId());
            map.put(StructureDescription.Orders.DATE_START, orderable.startDate().format(DateTimeFormatter.ISO_DATE));
            map.put(StructureDescription.Orders.DATE_FINISH, orderable.finishDate().format(DateTimeFormatter.ISO_DATE));
            map.put(StructureDescription.Orders.STATUS_ID, orderable.getStatus().getValue());
            map.put(StructureDescription.Orders.AUTOFINISH, 0);
            map.put(StructureDescription.Orders.IS_MONTHLY, 0);
            datas.add(map);
        }
        res.put(StructureDescription.Orders.TABLE_NAME, datas);
        return res;
    }
